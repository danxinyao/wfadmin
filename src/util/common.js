import { uniqBy, isEqual, cloneDeep, isArray } from 'lodash'
import Vue from 'vue'
...
/**
 * 数据行修改 0无改动 1新增 2修改 3删除
 * @param  {Array} initArr    初始数据，包括删除的状态
 * @param  {Array} currentArr 操作中的数据，包括新增的状态
 * @return {Array}            数据集合，包含0，1，2，3状态
 */
export const mergeArrayByRowStatus = function (initArr, currentArr) {
    const tempList = []
    // 修改的行、未修改的行
    if (isArray(initArr)) {
        initArr.forEach((item1) => {
            if (item1.rowStatus != 3) {
                for (let item2 of currentArr) {
                    if (item2.rowStatus != 1 && item2.rowStatus != 3) {
    ...
    if (isArray(currentArr)) {
        ...
        currentArr.forEach((item) => {
            if (item.rowStatus == 1 || item.rowStatus == 3) {
                tempList.push(item)
            }
        })
    }
...
/**
 * 根据key值排除key重复的数组
 * @param  {array} newArr 最新取得的数组
 * @param  {array} oldArr 旧的数组
 * @param  {string} key    key字符串
 * @return {array}        去重之后的数组
 */
export const getUniqueArrByKey = function (newArr, oldArr, key) {
    const arr = []
    if (isArray(newArr) && isArray(oldArr)) {
...
// 导出 调用
export const exportFile = function(type, query) {
    common.export(type, query).then((result) => {
        download(result)
    })
}
